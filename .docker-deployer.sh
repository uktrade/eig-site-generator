...
$DOCKER_PATH run --rm -i --name $DOCKER_IMAGE -e DIT_ENV=$DIT_ENV -e IIGB_BUILD=$IIGB_BUILD -e BUCKET=$BUCKET -e DID=$DID -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e GIT_ACCESS_TOKEN=$GIT_ACCESS_TOKEN -e NO_ROBOTS=$NO_ROBOTS -e NO_CACHE=$NO_CACHE $DOCKER_IMAGE /bin/bash -c "npm run deploy"
